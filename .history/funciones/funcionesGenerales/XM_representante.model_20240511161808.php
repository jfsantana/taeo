...
$idRepresentante = @$rs['result']['idHeaderNew'];

 //print("<pre>".print_r(($rs),true)."</pre>");die;

if (@$rs['status'] == 'OK') {
    $url = "onclick=\"enviarParametrosCRUD('admin/representanteList.php')\"";
    if(isset($_POST['idAprendiz'])){
      //hay que actualiza la tabla que relaciona al padre con el aprendiz
      // $idHeaderNew este es el nuevo id Represenatte
      // $_POST['idAprendiz']  id Aprenzid
      $parameter = [
        'idAprendiz' => $_POST['idAprendiz'],
        'idRepresentante' => $idHeaderNew
      ];
      $URL = "http://" . $_SERVER['HTTP_HOST'] . "/funciones/wsdl/aprendizRepresentante";
      $rs = API::POST($URL, $token, $_POST);
      $rs = API::JSON_TO_ARRAY($rs);
