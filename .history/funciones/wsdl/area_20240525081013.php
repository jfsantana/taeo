...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
   ...
   */
  if ($_GET['type']==1){
    $datosArray = $_area->getArea(@$_GET['idAreas']);
    header('Content-Type: application/json;charset=utf-8');
    echo json_encode($datosArray);
    http_response_code(200);
  }elseif(($_GET['type']==2)||(isset($_GET['idAreas']))){
    $datosArray = $_area->getNiveleByArea(@$_GET['idAreas']);
    header('Content-Type: application/json;charset=utf-8');
    echo json_encode($datosArray);
    http_response_code(200);
  }
  //quitar por que no es necesario despues de terminar la vista
  elseif(($_GET['type']==3)||(isset($_GET['idAreas']))){
    $datosArray = $_area->getRepresentanteByAprendiz(@$_GET['idAreas']);
    header('Content-Type: application/json;charset=utf-8');
    echo json_encode($datosArray);
    http_response_code(200);
  }
