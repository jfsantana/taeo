...
class representante extends conexion
{
  ...
  public function getObjetivosHeadere($idObjetivoHeader) //()
  {
    ...
    $query = "SELECT * FROM objetivo_item WHER

    E activo = 1 and objetivo_item.id_padre is null and objetivo_item.idHeader = 1 order by jerarquia;" . $this->tablaHeader . " $where ";
    //echo   $query; die;
  ...
  public function getIemsByHeader($idRepresentante) //()
  {
    ...
    if ($idRepresentante != '') {
      $where =  $where . " and t1.idHeader = " . $idRepresentante;
    }
