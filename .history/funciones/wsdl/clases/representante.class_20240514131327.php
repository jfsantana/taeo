...
class representante extends conexion
{
  ...
  public function getObjetivosHeadere($idRepresentante) //()
  {
    $where = " WHERE idRepresentante <> '' ";
    if ($idRepresentante != '') {
      $where =  $where . " and idRepresentante = " . $idRepresentante;
    }
    $query = "select *, case when representantes.activoRepresentante = 1 Then 'Activo' else 'Bloqeuado' end estado from " . $this->tabla . " $where ";

    return parent::ObtenerDatos($query);
  }

  public function getAprendizByRepresentante($idRepresentante) //()
  {
