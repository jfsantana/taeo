...
class objetivo extends conexion
{
  ...
  public function jerarquiaValidate($jerarquia){
    $jerarquiaValidate = $jerarquia;
    if (strlen($jerarquia)<2)
      return $jerarquiaValidate;
    else
    return 99;
  ...
  public function uploadXls($idHeader, $file){
    ...
    for ($row = 2; $row <= $highestRow; $row++) { //recorre las filas
      $idHeader;
      $jerarquia = '0.1';//$sheet->getCell('A' . $row)->getValue();
      $descripcion = $sheet->getCell('B' . $row)->getValue();


      $jerarquiaAnterior = $this->jerarquiaValidate($jerarquia);

      echo $jerarquiaAnterior ; die;
      //aqui hace el insert y retornando un ID que sera el  $id_padre siempre que se mantenga la jerarquia

      $id_padre=13;
      $jerarquiaAnterior=$jerarquia;

      echo $jerarquia. '- '.$descripcion."\n";
