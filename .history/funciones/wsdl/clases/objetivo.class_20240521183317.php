...
class objetivo extends conexion
{
  ...
  public function uploadXls($idHeader, $file){
    ...
    $actualiza=$this->updtaeid_padre(11);
              die;
    ...
    for ($row = 2; $row <= $highestRow; $row++) { //recorre las filas
      ...
      $insert = $this->insertItems($idHeader, $jerarquia, $id_padre, $descripcion,$activo);
        $line++;
             if ($line==15){
              $actualiza=$this->updtaeid_padre($idHeader);
              die;
             }
...
private function retornaPadre($jerarquia){
  if(strlen($jerarquia)==1){
    return null;
  }else{
    return $this->jerarquiaValidate($jerarquia);
  }
}
...
private function updtaeid_padre($idHeader){
  $nivel=0;
  $jerarquiaActual='';
  $jerarquiaAnterior='';
  $id_padre=0;
  ...
  $where = " WHERE activo = 1 ";
  if ($idHeader != '') {
    $where =  $where . " and idHeader = " . $idHeader;
  }
  $queryObjetivos = " SELECT * FROM objetivo_item   $where order by id  ";
  $objetivoArray = parent::ObtenerDatos($queryObjetivos);

  //recorre el objetivo para actualizar el padre
  foreach($objetivoArray as $datoObjetivo){

    $idPadreUpdate=$this->retornaPadre($datoObjetivo['jerarquia']);
    if ($idPadreUpdate==null){
      echo 'es null'."\n";
    }else{
      echo $idPadreUpdate."\n";
    }
    ...
    //print_r($datoObjetivo); die;

    // $jerarquiaActual=$this->jerarquiaValidate($datoObjetivo['jerarquia']);

    // if($jerarquiaAnterior==''){
    //   echo 'jerarquia de un solo nivel'."\n";
    //   $id_padreAnterior=$datoObjetivo['id'];
    //   $jerarquiaAnterior=$this->jerarquiaValidate($datoObjetivo['jerarquia']);

    // }else{
    //   echo 'jerarquia nievl N'."\n";
    //   if($jerarquiaAnterior==$jerarquiaActual){
    //     //actualizar el id_padre con el id_padreAnterior
    //     $queryUpdate = "update objetivo_item set id_padre='$id_padreAnterior' WHERE id=".$datoObjetivo['id'];
    //     echo $queryUpdate."\n";
    //    // $update = parent::nonQuery($queryUpdate);
    //   }else{
    //     if(strlen($datoObjetivo['jerarquia'])==1){
    //       $id_padreAnterior=$datoObjetivo['id'];
    //       $jerarquiaAnterior=$this->jerarquiaValidate($datoObjetivo['jerarquia']);
    //     }else{
    //       $queryUpdate = "update objetivo_item set id_padre='$id_padreAnterior' WHERE id=".$datoObjetivo['id'];
