...
class representante extends conexion
{
  ...
  {
    $where = " WHERE objetivo_item.activo = 1 ";
    if ($idRepresentante != '') {
      $where =  $where . " and objetivo_item.idHeader = " . $idRepresentante;
    }
    $query = "SELECT t1.jerarquia,CONCAT(REPEAT('    ', LENGTH(t1.jerarquia) - LENGTH(REPLACE(t1.jerarquia, '.', ''))), t1.descripcion) AS estructura
