...
class objetivo extends conexion
{
  ...
  public function getObjetivosHeadere($idObjetivoHeader) //()
  {
    ...
    $query = "select *, case when objetivo_header.activo = 1 Then 'Activo' else 'Bloqueado' end estado,  SUBSTRING_INDEX(observacionObjetivo, '\n', 1) as resumen from " . $this->tablaHeader . " $where ";
    //echo   $query; die;
  ...
  public function uploadXls($idHeader, $file){
    ...
    $eval=$this->updateId_padre($idHeader);


  }

  private function padreJerarquia($idHeader, $jerarquia){


    $query = "SELECT * FROM objetivo_item WHERE idHeader = $idHeader and jerarquia='$jerarquia'";
    $result = parent::ObtenerDatos($query);
    return $result[0]['id'];
  }

private function updateId_padre($idHeader){
    $jerarquiaPadre='';
    //query del objetivo
    $queryObjetivo = "SELECT * FROM objetivo_item WHERE activo = 1 and idHeader = $idHeader order by id";
    ...
    foreach($arrayObjetivo as $datoObjetivo){
      ...
      if(strlen($datoObjetivo['jerarquia'])==1){
        ...
        $idPadreJerarquia=$this->padreJerarquia($idHeader, $jerarquiaPadre);
