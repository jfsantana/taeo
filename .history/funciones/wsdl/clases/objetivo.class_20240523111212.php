      ...
      if ($arrayToken) {
        ...
        if (
          ...
          if($datos['mod']==1){//creacion del header y los items
            ...
            $ARRAY = [
              'idObjetivoHeader' => $this->idObjetivoHeader,
              'versionObjetivo' => $anteriorVersionObjetivo
          ];
            // debe pasar todos los items de estado activo a estado 0
            $TOKEN=$this->token;
            $URL1        = "http://" . $_SERVER['HTTP_HOST'] . "/funciones/wsdl/objetivoItem";

            $rs1         = API::PUT($URL1, $TOKEN, $ARRAY);
            $delItemByHeader  = API::JSON_TO_ARRAY($rs1);

            echo $URL1;
            echo $delItemByHeader;
            print("<pre>".print_r(json_encode($delItemByHeader),true)."</pre>"); die;
