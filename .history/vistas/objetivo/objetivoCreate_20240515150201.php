...
function generarDropdown($datos, $posicion = '', $nivel = 0) {
  echo '<select>';
  foreach ($datos as $dato) {
      if (strpos($dato['jerarquia'], $posicion) === 0 && substr_count($dato['jerarquia'], '.') == $nivel) {
          echo '<option value="' . $dato['jerarquia'] . '">' . $dato['descripcion'] . '</option>';
          generarDropdown($datos, $dato['jerarquia'], $nivel + 1);
      }
  }
  echo '</select>';
}
                      ...
                      foreach($arrayNodoPadre as $datosNodoPadre) {
                        ...
                        $datosNodoPadre['jerarquia'];  //valor numerico
                        $datosNodoPadre['descripcion'];  //descripcion
                        $datosNodoPadre['id_padre']; //id del padre
                        $datosNodoPadre['id']; //id del items 13 y 20
                        $jerarquia=$datosNodoPadre['jerarquia'];
                        $arrayhijos  = hijos($datosNodoPadre['id'], $token);

                        //imprime el pimer padre
                        echo'<tr data-widget="expandable-table" aria-expanded="false"><td>';
                        if(count($arrayhijos)>0){
                                  echo '<button type="button" class="btn btn-primary p-0">
                          ...
                          <tr class="expandable-body">
                                <td>
                                    <div class="p-0">
                                      <table class="table table-hover">
                                        <tbody>
                                          <?php foreach($arrayhijos as $datosArrayhijos){?>
                                            <tr data-widget="expandable-table" aria-expanded="false">
                                              <td>
                                                <?php $arrayhijosN1  = hijos($datosArrayhijos['id'], $token);

                                                if(count($arrayhijosN1)<1){
                                                  //imprime el HIJO NIVEL 1
                                                 echo  $datosArrayhijos['id'].'-'.$datosArrayhijos['jerarquia'].'-'.$datosArrayhijos['descripcion'];


                                                }else{
                                                  //imprime el HIJO NIVEL 2
                        ...
                        //tengo que saber si el proximo tiene hijos o no
