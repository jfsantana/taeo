...
if ($_POST['mod'] == 1) {
  ...
  //datos Representante
  $idnivelArea = @$_POST["id"];
  $token = $_SESSION['token'];
  $URL        = "http://" . $_SERVER['HTTP_HOST'] . "/funciones/wsdl/nivelArea?type=1&idnivelArea=$idnivelArea";
  ...
  if ($arrayUsuario[0]['activoRepresentante'] == 1)
    ...
    $URL        = "http://" . $_SERVER['HTTP_HOST'] . "/funciones/wsdl/representante?type=2&idnivelArea=$idnivelArea";
    $rs         = API::GET($URL, $token);
...
<form action="../funciones/funcionesGenerales/XM_representante.model.php" method="post" name="representante" id="representante">
  ...
  <input type="hidden" name="idnivelArea" value="<?php echo @$idnivelArea; ?>">
